...
/**
 * Стороны рамки ячейки: true рисовать, false не рисовать, не задано - работает
 * уровень ниже (настройка колонки, строки или общая).
 */
export type CellBordersOnOff = {
  top?: boolean;
  right?: boolean;
  bottom?: boolean;
  left?: boolean;
...
/**
 * Управление линиями сетки таблицы: включение и выключение. Уровни, от слабого
 * к сильному: общие настройки (vertical, horizontal), затем колонка
 * (columnConfig[i].verticalBorder) и строка (getHorizontalBorder), затем
 * ячейка (getCellBorder).
 */
export type GridLinesConfig<RowType = ObjectForExtending> = {
...
> = {
  ...
  /**
   * Управление линиями сетки таблицы: включение и выключение.
   *
   * Уровни, от слабого к сильному: общие настройки (vertical, horizontal),
   * затем колонка (columnConfig[i].verticalBorder) и строка
   * (getHorizontalBorder), затем ячейка (getCellBorder).
   *
   ...
   * по ячейкам.
   */
  gridLines?: GridLinesConfig<RowType>;
  /**
   * rowSize - размеры строк таблицы
   */
  rowSize?: Prettify<RowSize>;
...
\`\`\``,name:"of",required:!1,type:{name:"any"}},__forceInitialArgs:{defaultValue:null,description:"Internal prop to control if a story re-renders on args updates",name:"__forceInitialArgs",required:!1,type:{name:"boolean"}}}}}catch{}export{e as A,a as T};
