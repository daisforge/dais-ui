...
/**
 * Управление линиями сетки таблицы. Уровни (по возрастанию приоритета): глобально
 * (vertical/horizontal/color) → колонка (columnConfig[i].verticalBorder / borderColor)
 * → ячейка (getCellBorder).
 */
export type GridLinesConfig = {
  /** Глобально рисовать вертикальные линии между колонками. @default true */
  vertical?: boolean;
  /** Глобально рисовать горизонтальные линии между строками тела. @default true */
  horizontal?: boolean;
  ...
  /**
   * Пер-ячейковое переопределение сторон рамки. Возвращать undefined для ячеек без
   * переопределений. Включает посегментную отрисовку линий — применять осознанно.
   */
...
> = {
   ...
   *
   * Уровни (по возрастанию приоритета): глобально (vertical/horizontal/color) →
   * колонка (columnConfig[i].verticalBorder / borderColor) → ячейка (getCellBorder).
   *
   * \`vertical: false\` / \`horizontal: false\` убирают все вертикали / горизонтали тела.
   * \`color\` красит все линии (через тему). \`getCellBorder\` — точечная настройка сторон
   * рамки конкретной ячейки (вкл/выкл и цвет), включает посегментную отрисовку.
   */
