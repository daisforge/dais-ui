import{d as c}from"./react-D2T61mpp.js";import{ci as u}from"./vendor-DGOJ8IkJ.js";const e={"packages/ui-kit/src/components/AiAgentPopover/AiAgentPopover.types.ts$$$AiAgentPopoverProps":`type AiAgentPopoverProps = Omit<
  PopoverProps,
...
> & {
   ...
   */
  size?: PopupDFSize;
};`,"packages/ui-kit/src/components/PopupDF/types.ts$$$PopupDFSize":"type PopupDFSize = 's' | 'm' | 'l';","packages/ui-kit/src/components/PopupDF/types.ts$$$PopupDFHeaderProps":`type PopupDFHeaderProps = Omit<BoxProps, 'children'> & {
  /**
   * Заголовок в верхней левой части header.
   */
  title?: ReactNode;
...
};`,"packages/ui-kit/src/components/TableCanvas/types/table-config.type.ts$$$MergedCellsRegion":`type MergedCellsRegion = {
...
};`,"packages/ui-kit/src/components/TableCanvas/types/merged-cells.type.ts$$$MergedCellsAlign":`type MergedCellsAlign = {
...
> = {
...
};`,"packages/ui-kit/src/components/TableCanvas/feature-tree/types.ts$$$SubRows":`type SubRows<RowType, RowIdType> = {
  /**
   ...
   */
  switchDomMetadata?: DomMetadata;
};`,"packages/ui-kit/src/components/TableCanvas/feature-column-control/types.ts$$$HiddenColumnsIndicatorExpandInfo":`type HiddenColumnsIndicatorExpandInfo = {
...
/**
 * Управление линиями сетки таблицы. Уровни, от слабого к сильному: общие
 * настройки (vertical, horizontal, color), затем колонка
 * (columnConfig[i].verticalBorder, borderColor), затем ячейка (getCellBorder).
 */
export type GridLinesConfig = {
  ...
  horizontal?: boolean;
  /** Цвет всех линий сетки (перекрывает цвет из темы). */
  color?: string;
  /** Точечное управление горизонтальной линией сверху строки по её индексу. */
  getHorizontalBorder?: (rowIndex: number) => boolean;
  /**
   * Настройка сторон рамки у конкретной ячейки. Для ячеек без переопределений
   * возвращать undefined. Включает отрисовку линий с разбивкой по ячейкам,
   * поэтому функция должна быть быстрой и без лишних вычислений.
   */
...
> = {
  ...
  /**
   * Управление линиями сетки таблицы.
   *
   * Уровни, от слабого к сильному: общие настройки (vertical, horizontal, color),
   * затем колонка (columnConfig[i].verticalBorder, borderColor), затем ячейка
   * (getCellBorder).
   *
   * \`vertical: false\` и \`horizontal: false\` убирают все вертикальные или
   * горизонтальные линии тела. \`color\` красит все линии (через тему).
   * \`getCellBorder\` настраивает стороны рамки конкретной ячейки (вкл/выкл и цвет);
   * при его использовании линии рисуются с разбивкой по ячейкам.
   */
...
>;`,"packages/ui-kit/src/components/TableCanvas/feature-select-row/types.ts$$$SelectingRowConfig":`type SelectingRowConfig<RowType, RowIdType extends string | number> = {
  ...
  a: Row,
  b: Row,
) => number;`,"packages/ui-kit/src/components/TableCanvas/types/table-config.type.ts$$$SummaryRowsConfig":`type SummaryRowsConfig = {
  showDefault: boolean;
  showInControl: boolean;
} & {
  /**
...
> & {
  ...
  iconSize?: ComponentProps<typeof IconArrowLeft>['size'];
};`},a=({filePath:n,typeName:t,asString:r,preSource:i,postSource:s,...l})=>{if(!n||!t)return"";const p=`${n}$$$${t}`,d=(e==null?void 0:e[p])??"",o=`${i??""}${d}${s??""}`;return r?o:c.jsxDEV(u,{language:"tsx",code:o,...l},void 0,!1,{fileName:"/home/runner/work/dais-ui/dais-ui/packages/storybook/src/utils/TypeSourceViewer.tsx",lineNumber:43,columnNumber:10},void 0)};try{a.displayName="TypeSourceViewer",a.__docgenInfo={description:`Отображает определение типа (type или interface) из исходного файла.
Данные берутся из types.string.json (генерируется скриптом generate-storybook-types-as-string-json).`,displayName:"TypeSourceViewer",props:{filePath:{defaultValue:null,description:'@example "./packages/ui-kit/src/components/TableContract/types.ts"',name:"filePath",required:!0,type:{name:"string"}},typeName:{defaultValue:null,description:'@example "FetcherProps"',name:"typeName",required:!0,type:{name:"string"}},asString:{defaultValue:null,description:"Return value as string",name:"asString",required:!1,type:{name:"boolean"}},preSource:{defaultValue:null,description:"",name:"preSource",required:!1,type:{name:"string"}},postSource:{defaultValue:null,description:"",name:"postSource",required:!1,type:{name:"string"}},transform:{defaultValue:null,description:"Transform the detected source for display",name:"transform",required:!1,type:{name:"(code: string, storyContext: StoryContextForLoaders<Renderer, Args>) => string"}},type:{defaultValue:null,description:"Where to read the source code from, see `SourceType`",name:"type",required:!1,type:{name:"enum",value:[{value:'"auto"'},{value:'"code"'},{value:'"dynamic"'}]}},of:{defaultValue:null,description:`Pass the export defining a story to render its source
