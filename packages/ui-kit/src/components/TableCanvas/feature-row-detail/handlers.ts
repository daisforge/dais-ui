...
import { CellInfoGlideInstance } from '../TableGlideInstance/type';
...
export const getDetailPanelColSpanFunc =
  <RowType extends ObjectForExtending, SummaryRowType>({
    ...
    columnColSpan:
      | undefined
      | number
      | ((args: CellInfoGlideInstance<RowType, SummaryRowType>) => number);
  }) =>
  (args: CellInfoGlideInstance<RowType, SummaryRowType>): number => {
    const isDetailPanel = isDetailPanelRow(args.row);
    ...
    if (typeof columnColSpan === 'function') return columnColSpan(args);

    return columnColSpan ?? 1;
  };
