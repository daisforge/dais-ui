import type { DrawArgs, InnerGridCell } from '@glideappsfinal/glide-data-grid';
...
export const easeOutCubic: AnimationEasing = (t) => 1 - (1 - t) ** 3;

export const easeInOutCubic: AnimationEasing = (t) =>
  t < 0.5 ? 4 * t * t * t : 1 - (-2 * t + 2) ** 3 / 2;

type RendererDrawStateTuple = [
  RendererAnimationState | undefined,
  (state: RendererAnimationState) => void
];
...
export function animateNumericValue<T extends InnerGridCell>(
  ...
  const duration =
    options.duration ?? animationState.duration ?? DEFAULT_DURATION;
  ...
  if (animationState.startTime != null) {
    ...
    animationState.value =
      ...
      (animationState.target - animationState.startValue) * easedProgress;
