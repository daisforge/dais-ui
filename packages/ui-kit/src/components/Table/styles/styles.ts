...
export const HEIGHT_CONTROL_BLOCK = 41;
...
const getHeightOfTable = (
  ...
  if (typeof height === 'number' || typeof height === 'string') {
    ...
    const heightOfControlBlock = isHaveControlBlock ? HEIGHT_CONTROL_BLOCK : 0;

    // Если кнопка коллапсинга сверху - добавляем еще один HEIGHT_CONTROL_BLOCK
    const heightOfCollapseBlockAbove =
      collapseButtonPlacement === 'above' ? HEIGHT_CONTROL_BLOCK : 0;
