...
import { INITIAL_POSITION_CALCULATION_DELAY } from '../constants';
import type { MassActionsButtonProps } from '../types';

/**
 * Хук для управления эффектами панели массовых действий
 */
export const useMassActionsEffects = ({
  ...
  }, [containerWidth, calculatePosition, calculateCompression]);
  ...
  useEffect(() => {
    if (isVisible && !isCollapsed) {
      ...
      const timeoutId = setTimeout(() => {
        calculateCompression();
        calculatePosition();
      }, INITIAL_POSITION_CALCULATION_DELAY);
  ...
  useEffect(
    ...
    [isVisible, selectedCount, setIsCollapsed]
  );
};
