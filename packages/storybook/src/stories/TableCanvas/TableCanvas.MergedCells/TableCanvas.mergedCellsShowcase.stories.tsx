...
/**
 * ЕДИНЫЙ БОЛЬШОЙ ЭТАЛОН: как внешний потребитель отдаёт иерархические данные,
 * чтобы увидеть объединённые ячейки, и пользуется всем набором сразу — пагинация,
 * сортировка, фильтрация, глобальный поиск, resize, пиннинг, чекбоксы (выбор строк),
 * размер строки, настройка колонок, fullscreen, редактирование.
 *
 * Подход X («merged-плоский»). Потребитель ВЛАДЕЕТ деревом (дивизион / управление /
 * команда / сотрудник). Дерево он разворачивает в ЛИСТЬЯ (по строке на сотрудника) и
 * денормализует путь предков в поля-ключи (divPath / unitPath / teamPath). Колонки-предки
 * объединяются по этим ключам пути через mergeByCellValues, поэтому одинаковые имена в
 * разных ветках НЕ сливаются.
 *
 * Владение данными на стороне потребителя. Пагинация, сортировка, фильтрация и поиск
 * помечены manual* и считаются здесь (эмуляция бэка): запрос -> фильтры -> сортировка ->
 * страница. Таблица рисует полученную страницу и объединяет ячейки.
 *
 * Пиннинг и группы заголовков вместе. По умолчанию обёртка гасит пиннинг при активных
 * группах (`pinning = enable && !columnsGroupingIsActive`), но это только ДЕФОЛТ: явный
 * `columnsControl.pinning: true` перекрывает его (user-конфиг мёржится поверх дефолта).
 * Поэтому здесь одновременно и группы (над мерами), и пины (на иерархии). Иерархо-колонки
 * оставлены плоскими, группы — только над мерами, чтобы шапка групп не пересекала границу
 * frozen-области.
 *
 * Сортировка сохраняет объединения: сортируем листья ВНУТРИ команды, порядок блоков
 * команд не трогаем, поэтому строки одной команды остаются соседними и объединения не
 * рассыпаются (позиционный merge). Сортировка иерархо-колонок (дивизион/команда) —
 * отдельная тема (нужна сортировка БЛОКОВ, не листьев), пока не реализована.
 */
const meta: Meta = {
  ...
  const shared = {
    ...
    pagination: {
      ...
      onChange(
        ...
        if (typeof nextPerPage === 'number') setPerPage(nextPerPage);
        scrollToTop();
      },
    },
  };
    ...
    return (
      <div>
        <StoryHint>
          ...
          Продажи → План/Факт). Колонки иерархии закреплены слева (пины): при
          горизонтальной прокрутке мер оргструктура остаётся на месте. Пины и
          группы вместе — через явный `pinning: true` (дефолт их гасит).
          Пагинация, сортировка, фильтрация и поиск считаются на стороне
        ...
        <TableCanvas
          tableConfig={{
            ...
            pagination: {
              ...
              onChange(nextPage, _perPage, scrollToTop) {
                if (typeof nextPage === 'number') setPage(nextPage);
                scrollToTop();
              },
            },
