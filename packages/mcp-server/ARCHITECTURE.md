...
Их MCP (`@salutejs/sdds-mcp`) сам работает поверх статического JSON-бандла, который генерирует скрипт `generate-mcp-data` в репозитории `plasma/website/sdds-finai-docs` (схема на компонент: `{name, package, category, summary, api:{props}, examples}`). Вместо подключения к их MCP как к живому процессу — **вендорим тот же JSON** к себе:

- `vendorAtomicData.ts` тянет его с CDN `https://plasma.sberdevices.ru/mcp/<lib>/<version>/` в `packages/mcp-server/vendor/atomic-mcp-data/` (закоммиченный снэпшот; в рантайме сервера сеть не используется). Подробности механики — §1.6c. Прежний режим `--source <путь-к-их-mcpData>` сохранён как офлайн-путь.
...
**Поправка (T18):** блокировка касалась только второго скрипта. Первый, `generate-index`, на голом чекауте работает: раскрытие `<PropsTable>` у него обёрнуто в `fs.existsSync`, поэтому без артефакта докусауруса он не падает, а пропускает таблицы — см. §1.6d.
...
Осталась открытой только первая часть T15 (`extractProps` берёт со страницы одну таблицу — 14 таблиц не попадают в снэпшот в принципе): она чинится на стороне атомарной команды, перевендоринг сам по себе её не закрывает.

### 1.6c Загрузка снэпшота с CDN (T17)

До T17 снэпшот обновлялся только из локального чекаута доков (`--source <путь-к-их-mcpData>`), а получить этот путь можно было, лишь прогнав у них docusaurus-сборку. Из-за этого снэпшот застревал на месяцы, и автоматизировать релиз `@daisforge/ui-mcp` было нечем.

Оказалось, что тот же бандл публикуется на CDN: `https://plasma.sberdevices.ru/mcp/<lib>/<version>/manifest.json`. Это **не** обходной путь: в npm-пакете `@salutejs/sdds-mcp` лежит только `dist/` без данных, а его `dist/config.js` содержит ровно этот `MANIFEST_BASE_URL` — то есть их собственный MCP-сервер читает те же самые адреса. Схема файлов совпадает с вендоренной побайтово, так что `mergeAtomicData.ts`/`vendorExamples.ts` не менялись вовсе.

Что важно в реализации:

- **Версия — установленная, а не `latest`.** Скрипт читает `node_modules/@salutejs/sdds-finai/package.json`. Иначе индекс начинает обещать пропсы, которых у потребителя ещё нет (ровно тот `index-drift`, что считает `ab/drift.mjs`).
- **Редиректы не следуем.** Неопубликованной версии на CDN может не быть вовсе (`0.355.0`, `0.351.0`, `0.350.0` → нет; `0.340.0`, `0.359.0`, `0.360.0` → есть), и такой запрос отвечает **301 на главную страницу, которая отдаёт HTML с кодом 200**. С `redirect: 'follow'` (как в клиенте атомарной команды) ответ выглядит успешным, и в снэпшот уехал бы HTML. Отсюда `redirect: 'manual'` + проверка формы каждого файла до записи.
- **Отсутствующая версия не валит релиз.** `--on-missing=keep` (по умолчанию) оставляет закоммиченный снэпшот и печатает `::warning::`. Старые валидные данные лучше, чем чужая версия или пустота.
- **Сначала стейджинг, потом снос.** Прежняя версия начинала с `rmSync(VENDOR_DIR)`, и обрыв сети посреди прогона оставлял репозиторий без вендорных данных. Теперь всё скачивается и валидируется во временном каталоге, а замена происходит одним шагом в конце; если содержимое совпало с текущим — файлы не переписываются вообще.
- **Даунгрейд запрещён** без `--force` (сравнение `manifest.version` со снэпшотом) — раньше более старый чекаут доков молча обеднял снэпшот.
- **Секции берутся из манифеста**, а не из захардкоженного списка, поэтому кроме `components`/`beta` снэпшот теперь несёт и гайды (`form`, `utils`, `tokens`, `how-to-icons`, `intro`, `FAQ`, `next`, `react_17`, `mcp`). В индекс они пока не попадают — отдельная задача; `mergeAtomicData`/`vendorUsageExamples` читают только свои два подкаталога по именам и на соседей не реагируют.
- **`provenance.json`** (пишем мы, с CDN не приходит) фиксирует `resolvedVersion`/`installedLibVersion`/`sourceUrl`/`fetchedAt`, счётчики по секциям и sha256 каждого файла.

Итог первого прогона: **`0.355.0 → 0.360.0`**, новый `Divider` (73 → 74 компонента), новые пропсы у 16 компонентов (`NumberInput` +9, `Sheet` +6, `Range` +3), `inheritedProps` 1766 → 1822, `atomicDataMissing` 34 → 31.

**Цена этого апгрейда — +8 диагностик `validateIndex`** (574/444 → 582/452 на одних и тех же исходниках ui-kit; сверено прогоном со старым снэпшотом). Причина не в скрипте: бандл доков `0.360.0` собран с более свежего исходника, чем одноимённый npm-пакет, и часть новых пропсов (`Sheet.snapPoints`, `Modal.focusTrapSelectors`, `DatePicker.onChangeVisibleDate`, `Popover.stretchHeight`, `Skeleton.animationDuration`) в типах установленного `@salutejs/sdds-finai@0.360.0` отсутствует — проверено грепом по его `.d.ts`. То есть пин по версии необходим, но недостаточен: одинаковый номер версии у доков и у пакета не означает одинакового содержимого. Если это станет мешать — следующий шаг не в вендоринге, а в сверке вендорных пропсов с тайпчекером на этапе `mergeAtomicData`.

### 1.6d Документация атомарки (`harvestAtomicDocs.ts`, T18)

Бандл на CDN несёт пропсы и примеры, но **не несёт прозы**: их генератор режет `summary` компонента по 300 символов. Замер по снэпшоту `0.360.0`: медиана **44 символа**, у 34 из 74 компонентов остаётся фактически голое имя (`Divider` — пустая строка, `Cell`/`Link`/`Mask` — 4 символа), а два самых длинных (`Range`, `Tour`) обрываются ровно на 300-м символе посреди слова. Для компонентов ui-kit дыру закрывает Storybook (`mergeMeta`, 41 компонент), но 117 чистых реэкспортов атомов своей страницы не имеют — у них не было ничего, кроме курированных одной-двух строк из `atomic-curated-meta.json`.

Текст при этом существует: их пайплайн двухступенчатый, и **первый** скрипт (`generate-index`) кладёт в `index.json` страницу целиком. Обрезка живёт во втором (`generate-mcp-data`), который и собирает то, что выкладывается на CDN. Поэтому источник прозы — первый скрипт, запускаемый на их чекауте:

1. **sparse + partial clone тега** `@salutejs/sdds-finai@<установленная версия>` — забираем только `website/sdds-finai-docs`. Замер: **17 МБ и ~6 с** против 990 МБ и ~52 с у полного клона. Тег под нужную версию существует (проверено для `0.360.0`), так что проза и пропсы всегда описывают одну и ту же версию атома.
2. **Пять пакетов, которые импортирует их скрипт** (`gray-matter`, `unified`, `remark-parse`, `remark-mdx`, `unist-util-visit`), ставятся в ОТДЕЛЬНЫЙ каталог и подсовываются симлинком. `npm install` прямо в `website/sdds-finai-docs` подтянул бы весь `package.json` докусауруса — сотни пакетов и минуты; так — **3.4 с**.
3. **`node ./scripts/generateIndex/index.mjs`** — **0.5 с**. Сборка докусауруса не нужна: раскрытие `<PropsTable>` обёрнуто у них в `fs.existsSync`, без артефакта скрипт просто логирует «File does not exist» и идёт дальше. Нам это и нужно — пропсы у нас из ts-morph и с CDN, не хватало только текста.

Итого весь сбор — **~10 секунд** (замерено end-to-end), результат: 87 страниц, **706 685 символов**. По компонентам медиана вырастает с 44 символов до **5 902** (у `Button` — 100 → 6161), максимум 41 406.

Раскладка (`vendor/atomic-docs/`) повторяет структуру CDN-снэпшота: `components/button.json`, `beta/popover.json`, `form/react-hook-form.json` и т.д., имя файла — slug из канонического URL страницы (87 уникальных, дубли ловятся явной проверкой). Запись — `{name, slug, category, url, content}`; `url` — готовая ссылка «читать в доках», которой для атомов не было вовсе. Рядом `pages.json` (перечень страниц с размерами) и `provenance.json` (repo, ref, установленная версия, время, счётчики по категориям, sha256 по файлам).

Политика отказов та же, что у вендоринга с CDN: недоступен GitHub, нет тега под версию, изменилась раскладка их репозитория — `::warning::` и каталог остаётся нетронутым (`--on-missing=keep`, по умолчанию). Стейджинг и атомарная замена — общие с ним (`vendorShared.ts`).

**Что этот шаг НЕ делает:** проза складывается в снэпшот, но в индекс пока не попадает и тулами не отдаётся. Выдача — отдельное решение: медиана 5.9k символов на компонент не влезает в карточку `get_component` (бюджет ответа 25k на весь ответ), поэтому нужен отдельный тул либо клип с ссылкой на `url`.
...
Все три уровня протестированы вживую (см. историю сессии): реальный сценарий "`@daisforge/ui` не установлен вовсе" — сервер не падает, отдаёт `bundled`-индекс и корректно предупреждает агента, что стоит вызвать `get_installation_guide` (в T8 этот тул был ресурсом `daisforge-ui://catalog/installation-guide`, в T16 вернулся тулом — см. §4).
...
| `list_categories()` | категории с разбивкой по типам; аргументов нет |
| `get_installation_guide()` | гайд по установке из `guides.installation`; аргументов нет |

**Одиннадцать инструментов и ни одного ресурса — итог T8 и последовавшего отката T16.** T8 перевёл два последних тула таблицы в MCP-ресурсы (`daisforge-ui://catalog/categories`, `daisforge-ui://catalog/installation-guide`) по формально верному признаку: аргументов не принимают, контент от вызова не зависит — типовой ресурс, а не тул-с-параметрами. На практике это сделало их недостижимыми: **список тулов с описаниями приходит в контекст агента сам при каждом запросе, список ресурсов — нет**, его нужно запросить отдельным `resources/list`, а повода это делать у агента не возникает. Подтверждение из транскриптов A/B-замера (§9, 40 прогонов с доступом к MCP, 279 вызовов инструментов): `resources/read` — ноль вызовов, URI обоих ресурсов не упоминаются ни разу. T16 вернул оба тулами с пустой схемой аргументов (`{}`); `registerResource`/`toResourceResult` удалены, capability `resources` сервером не объявляется — `resources/list` отвечает `-32601 Method not found`, что честнее пустого списка.

Единственная часть T8, пережившая откат: значения `category` (три — `"Локальные компоненты"`/`"Композиции"`/`"Формы"`) остались продублированы прямо в описании тула `list_components`, так что фильтр можно применить и без вызова `list_categories`.
...
- **`atomicDataMissing: true` у 31 записи** (37 до T15 → 35 после разметки страниц → 34 после перевендоринга на `0.355.0` → 31 после T17, когда на `0.360.0` появилась страница `Divider` и закрыла `Divider`/`ModalDFDivider`/`WidgetDivider`; `ToolbarDivider` остался — его `atomicBase` совпадает с его собственным именем, а такой страницы нет; см. §1.6b/§1.6c) — вендорных пропсов нет по двум разным причинам, и обе не лечатся на нашей стороне: у 11 записей таблица пропсов в доках атомарной команды ЕСТЬ, но не первая на своей странице, и снэпшот её теряет (`CalendarBase`/`CalendarBaseRange`/`CalendarDouble`/`CalendarDoubleRange`, `DatePickerRange`, `NotificationsProvider`, `SegmentItem`, `TextSkeleton`, `TabItem`, `TabsController`, `FiltersActionsTabItem`) — закроется только перегенерацией снэпшота с исправленным `extractProps`; у остальных 23 таблицы нет в доках вовсе (`Divider` и три его обёртки `ModalDFDivider`/`ToolbarDivider`/`WidgetDivider`, compound-части `DrawerHeader`/`DrawerContent`/`DrawerFooter`, `SegmentIconItem`, `IconTabItem`, `ToastProvider`, `Typography` и 13 атомов типографики — страница `Typography.mdx` документирует семью прозой и примерами, ни одной `<PropsTable>` на ней нет). Пропсы у всех 31 при этом есть — из собственного резолва ts-morph, с настоящими русскими описаниями и корректным `required`, чего вендор не даёт вовсе (см. §1.6).
- **Бандл доков может расходиться с одноимённым npm-пакетом.** После T17 снэпшот тянется с CDN по версии установленного атома, но `0.360.0` в доках оказался собран с более свежего исходника, чем `@salutejs/sdds-finai@0.360.0` в npm: `Sheet.snapPoints`, `Modal.focusTrapSelectors`, `DatePicker.onChangeVisibleDate`, `Popover.stretchHeight`, `Skeleton.animationDuration` в его `.d.ts` отсутствуют (+8 диагностик `validateIndex`). Закрывается сверкой вендорных пропсов с тайпчекером на этапе `mergeAtomicData` — отдельная задача, см. §1.6c.
- **Дефект `extractProps` у атомарной команды** (со страницы доков в бандл попадает только первая таблица пропсов — 14 таблиц теряются) перевендорингом не лечится и остаётся за ними; §1.6b.
...
npm run mcp:vendor-atomic:check  # сухой прогон вендоринга: что приедет с CDN, ничего не пишется
npm run mcp:vendor-atomic        # обновить снэпшот атомарных пропсов с CDN (§1.6c; офлайн-режим — флаг --source)
...
Руками вендоринг запускать в общем случае не нужно: тот же `npm run mcp:vendor-atomic` выполняется шагом релизного workflow перед сборкой индекса (§8).

## 8. Релиз (T17)

`@daisforge/ui-mcp` публикуется **хвостовым job'ом релиза `@daisforge/ui`**: `publish-npm.yml` (триггер — пуш в `main`) после публикации библиотеки вызывает `publish-mcp-server.yml` через `workflow_call`. Ручной `workflow_dispatch` у второго сохранён.

Почему цепочкой, а не отдельным push-триггером: оба workflow пушат релизные коммиты в `main` (lerna-бамп и `chore(release-mcp)`), и параллельные прогоны давали бы non-fast-forward. Отсюда же вход `ref` — вызванный workflow чекаутит `main`, а не `github.sha` события, который к тому моменту указывает на состояние до lerna-коммита.

Условие запуска — `released || mcp_touched`. Второй признак обязателен: `mcp-server` не входит в npm workspaces, лерна его не версионирует, и мерж, тронувший только `packages/mcp-server/**`, иначе не приводил бы к релизу ui-mcp никогда.

Перед сборкой индекса тот же job обновляет вендорный снэпшот с CDN (§1.6c). Шаг намеренно не жёсткий: недоступный CDN или неопубликованная версия дают `::warning::` и оставляют закоммиченный снэпшот — релиз собирается на нём, а не падает из-за чужой инфраструктуры. Изменившиеся вендорные файлы уезжают в тот же релизный коммит, что и бамп версии с пересобранным индексом.

## 9. A/B-замер: помогает ли индекс (T10)
...
### 9.1 Стенд — потребительский, а не репозиторный
...
### 9.3 Результат (20 задач × 3 руки, sonnet, 60 прогонов, $9.99)
