...
- `list_categories()` — категории каталога с количеством компонентов по типам; значения `category` отсюда подходят как фильтр для `list_components` (они же продублированы прямо в описании `list_components`, чтобы фильтр можно было применить без этого вызова)
- `get_installation_guide()` — установка пакета, подключение стилей и токенов, использование компонентов и иконок; на него ссылается `dataVersionNotice`, когда `@daisforge/ui` в проекте не установлен

Одиннадцать тулов. Ресурсов (`resources/list`/`resources/read`) сервер не объявляет вовсе: `list_categories`/`get_installation_guide` были переведены в ресурсы в T8 (аргументов не принимают, контент не зависит от вызова) и возвращены в тулы в T16 — **ресурс агент может прочитать, только зная его URI заранее**, а список ресурсов, в отличие от списка тулов, сам в контекст агента не приходит. В транскриптах A/B-замера (40 прогонов с MCP, 279 вызовов инструментов) не было ни одного чтения ресурса.
