...
npm run mcp:vendor-refresh        # то же + следом собрать документацию (см. ниже)
```
...
## Документация атомарки (`vendor/atomic-docs/`)

В бандле на CDN прозы нет: их генератор режет `summary` компонента по 300 символов — медиана **44 символа**, у 34 из 74 компонентов остаётся голое имя. Текст берётся отдельно, из **первого** скрипта их же пайплайна (`generate-index`), где страница лежит целиком:

```bash
npm run mcp:harvest-docs:check   # сухой прогон
npm run mcp:harvest-docs         # собрать документацию
```

Скрипт делает sparse-чекаут тега `@salutejs/sdds-finai@<установленная версия>` (только каталог доков — 17 МБ), ставит пять парсер-пакетов в отдельную папку, прогоняет их `generateIndex` и раскладывает результат в `vendor/atomic-docs/<slug>.json` (`{name, slug, category, url, content}`). Весь цикл — **~10 секунд**, 87 страниц, 706 685 символов; медиана текста на компонент вырастает с 44 до **5 902** символов.

Сборка докусауруса не нужна: раскрытие `<PropsTable>` у них опционально (`fs.existsSync`), а пропсы у нас и так из ts-morph и с CDN. Недоступен GitHub или нет тега под версию — `::warning::` и каталог остаётся прежним (`--on-missing=keep`), релиз не падает. Флаги: `--version`, `--ref`, `--repo`, `--on-missing keep|fail`, `--dry-run`.

Обратная операция — склеить каталог в один файл ровно того формата, что отдаёт их `generateIndex` (`[{pageContent, metadata}]`, готово к заливке в векторную базу):

```bash
npm run mcp:export-docs                      # → packages/mcp-server/.probe/atomic-docs-index.json
npm run mcp:export-docs -- --out /путь.json  # свой путь
```

По умолчанию пишет в `.probe/` (в `.gitignore`), чтобы в репозитории не лежала вторая копия тех же данных. Сверено с оригинальным выводом их скрипта: metadata совпадает 87/87, контент 83/87 — расхождение ровно на 4 страницы, которые менялись между тегом версии и `master`.

**Пока это только сырьё**: в индекс проза не попадает и тулами не отдаётся — медиана 5.9k символов на компонент не помещается в карточку `get_component` при бюджете ответа 25k. Как отдавать (отдельный тул или клип со ссылкой на `url`) — отдельная задача.
...
Последний замер (20 задач × 3 руки, sonnet): вызовов инструментов до первой строчки кода — **24.3 без MCP против 7.1 с MCP**, доля ответов, проходящих `tsc`, — **60% против 85%**. Разбор, оговорки и разрез по сложности задач — `ARCHITECTURE.md` §9, подробности харнесса — `ab/README.md`.
