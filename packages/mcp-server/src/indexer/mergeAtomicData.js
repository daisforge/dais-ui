...
function loadAtomicComponent(atomicBase) {
 ...
 */
export function mergeAtomicData(record) {
  ...
  const inheritedProps = (atomicComponent.api?.props || []).map((p) => ({
    ...p,
    inheritedFrom: record.atomicBase,
    inheritedPackage: '@salutejs/sdds-finai',
  }));

  const inheritedNames = new Set(inheritedProps.map((p) => p.name));
  const ownProps = (record.props || []).filter((p) => !inheritedNames.has(p.name));
