...
/**
 * "Разумный размер" примера — усечение длинного JSX-блока, не поиск валидного
 * среза. 700 символов (изначальное значение) резали ровно по самому ценному:
 * у `TableCanvas` обрыв приходился на `columnConfig`, то есть на конфигурацию
 * колонок — единственное место примера, которое агент не может додумать. В
 * реальном логе это и дало ошибку компиляции: `renderCell` был написан
 * наугад. Ответ тула бюджетируется отдельно (RESPONSE_BUDGET_CHARS = 25000 на
 * один пример), так что 2400 в него укладывается с запасом.
 */
const MAX_USAGE_CODE_CHARS = 2400;
