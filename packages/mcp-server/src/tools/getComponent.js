import { findComponent, listComponentFeatures, summarizeProp } from './shared.js';
...
export function getComponent(index, { name } = {}) {
  ...
  if (!record) {
    return { error: `Компонент "${name}" не найден. Используйте search_components или list_components.` };
  }
  if (record.error) {
  ...
  const exampleTitles = (record.examples || []).map((e) => e.displayName || e.exportName);
  ...
  return {
    ...
    exampleTitles,
    features: features.map((f) => ({ feature: f.feature, legacy: f.legacy || undefined })),
    dataVersionNotice: index.dataVersionNotice,
    nextSteps:
      'get_component_props({name}) — полные пропсы; get_component_examples({name}) — примеры кода' +
